#Class configuring Citrix Director service account and associated SPN in Active Directory
class xd7director::serviceaccounts inherits xd7director {
  ...
  dsc_xaduser{'SvcDirectorAccount':
    ...
    dsc_domainadministratorcredential => {'user' => $xd7director::setup_svc_username, 'password' => $xd7director::setup_svc_password},
    dsc_username                      => $xd7director::director_svc_username,
    dsc_password                      => {'user' => $xd7director::director_svc_username, 'password' => $xd7director::director_svc_password},
    dsc_ensure                        => 'Present',
  ...
  if $xd7director::loadbalandeddirector {
    dsc_xadserviceprincipalname{'DirectorLoadBalancedSPN':
      dsc_account              => $xd7director::director_svc_username,
      dsc_serviceprincipalname => "http/${xd7director::loadbalanceddirectorfqdn}",
      dsc_ensure               => present,
      dsc_psdscrunascredential => {'user' => $xd7director::setup_svc_username, 'password' => $xd7director::setup_svc_password},
  ...
  dsc_xgroup{'SvcDirectorIISGroup':
    ...
    dsc_ensure           => 'Present',
    dsc_memberstoinclude => "${facts['domainnetbiosname']}\\${xd7director::director_svc_username}",
  ...
  dsc_userrightsassignment{'AssignLogOnAsBatchToDirector':
    ...
    dsc_identity => ["${facts['domainnetbiosname']}\\${xd7director::director_svc_username}", 'Administrators', 'Backup Operators', 'Performance Log Users'],
    require      => Dsc_xaduser['SvcDirectorAccount']
  }
  ...
  dsc_userrightsassignment{'AssignImpersonateAfterAuthenticationToDirector':
    ...
    dsc_identity => ["${facts['domainnetbiosname']}\\${xd7director::director_svc_username}", 'Administrators', 'Local Service', 'Network Service', 'Service'],
    require      => Dsc_xaduser['SvcDirectorAccount']
  }
